...
from utils.encoding import decode_values, get_reverse_map
...
def decode_labels(values, target_col):
    return np.array(decode_values(values, target_col))
...
if reverse_map:
    class_labels = [reverse_map[k] for k in sorted(reverse_map.keys())]
else:
...
with st.expander("4.2 Data Preparation (Preprocessing)", expanded=True):
    ...
    if not encoding_table.empty:
        render_table(
            encoding_table,
            "Table 4.3 Encoding Mapping (Standard)",
...
with st.expander("4.4 Split 80:20 (Training/Testing)", expanded=True):
    ...
    y_train_labels = decode_labels(y_train, target_col)
    y_test_labels = decode_labels(y_test, target_col)
...
with st.expander("4.7 Model Testing", expanded=True):
    ...
    if not training_state:
        ...
        y_test_labels = decode_labels(training_state["y_test"], target_col)
        y_pred_labels = decode_labels(training_state["y_pred"], target_col)
