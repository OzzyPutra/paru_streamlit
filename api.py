...
from utils.encoding import get_reverse_map
...
def _set_pack(new_pack):
    global pack, model, target_col, feature_cols
    pack = new_pack
    if new_pack:
...
def _pack_info(active_pack):
    ...
    reverse_map = get_reverse_map(target)
    if reverse_map:
        classes = [str(reverse_map[k]) for k in sorted(reverse_map.keys())]
    else:
